	...
	function ewp_gsc_create_sitewide_metabox() {
		...
		foreach( $ewp_gsc_list as $customization => $description ) {
			...
			if ( strpos( $customization, 'custom_' ) !== FALSE ) {
				echo '<div style="float:left; width:40%;"><label for="' . GENESIS_SETTINGS_FIELD . '[ewp_gsc_' . $customization . '_text]" >' . $description . '</label></div><div style="float:left;"><input type="text" name="' . GENESIS_SETTINGS_FIELD . '[ewp_gsc_' . $customization . '_text]" id="' . GENESIS_SETTINGS_FIELD . '[ewp_gsc_' . $customization . '_text]" size="40" value="' . genesis_get_option( 'ewp_gsc_' . $customization . '_text' ) . '" /></div><br />';
			} else {
